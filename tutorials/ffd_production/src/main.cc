...
struct ProductionConfig
{
    ...
    std::string mode = "entropy"; // entropy, hm, or both
    int hm_measure_every = 5;
    int entropy_measure_every = 5;
    ...
    std::string out_dir = "tutorials/ffd_production/data";
    std::string tag = "";
...
std::string readOptionValue(int &i, int argc, char **argv, const std::string &arg)
{
    auto eq = arg.find('=');
...
std::string optionKey(const std::string &arg)
{
    auto eq = arg.find('=');
    if (eq == std::string::npos)
    {
...
void printProductionHelp()
{
    std::cout
        << "FFD production TEBD runner\n\n"
        << "Usage:\n"
        << "  ffd_production [options]\n\n"
        << "Core options:\n"
        << "  --N 40                       Number of sites\n"
        << "  --dt 0.1                     Time step\n"
        << "  --steps 50                   Number of TEBD steps\n"
        << "  --chi 128                    Maximum bond dimension\n"
        << "  --cutoff 1e-10               SVD discarded-fraction cutoff\n"
        << "  --order 2                    Trotter order: 1 or 2\n\n"
        << "Quench options:\n"
        << "  --theta 0.3926990817         Initial state angle\n"
        << "  --alpha 1 --beta 2 --gamma 3 Staggered couplings\n\n"
        << "Measurement options:\n"
        << "  --mode entropy               entropy, hm, or both\n"
        << "  --ells 5,10,15,20            Entanglement subsystem sizes\n"
        << "  --ell-count 5                Auto-generate this many ell values up to N/2\n"
        << "  --probes 18,19,20,21,22      h_m probe sites\n"
        << "  --hm-measure-every 5         h_m measurement cadence\n"
        << "  --entropy-measure-every 5    Entropy measurement cadence\n"
        << "  --measure-every 5            Set both measurement cadences\n\n"
        << "Output options:\n"
        << "  --out-dir path               Output directory, must already exist\n"
        << "  --tag scanA                  Extra label in output filenames\n"
        << "  --overwrite                  Allow overwriting output files\n"
        << "  --progress-every 10          Print progress every this many steps\n\n"
        << "Examples:\n"
        << "  ./ffd_production --mode entropy --N 40 --chi 128 --dt 0.1 --steps 50 --order 2 --ells 5,10,15,20 --tag scanA\n"
        << "  ./ffd_production --mode hm --N 60 --chi 192 --dt 0.05 --steps 100 --order 2 --probes 28,29,30,31,32\n";
}

ProductionConfig parseProductionConfig(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        std::string key = optionKey(arg);
        std::string value = readOptionValue(i, argc, argv, arg);
        ...
        if (key == "N")
            ...
            cfg.chi_max = std::stoi(value);
        else if (key == "cutoff")
            cfg.cutoff = std::stod(value);
        else if (key == "order")
            cfg.order = std::stoi(value);
        else if (key == "theta")
            ...
            cfg.mode = value;
        else if (key == "hm-measure-every")
            cfg.hm_measure_every = std::stoi(value);
        else if (key == "entropy-measure-every")
            cfg.entropy_measure_every = std::stoi(value);
        else if (key == "measure-every")
        {
            cfg.hm_measure_every = std::stoi(value);
            cfg.entropy_measure_every = cfg.hm_measure_every;
        }
        else if (key == "progress-every")
            cfg.progress_every = std::stoi(value);
        else if (key == "ells")
            cfg.ells = parseIntList(value);
        else if (key == "ell-count")
            cfg.ell_count = std::stoi(value);
        else if (key == "probes")
            cfg.probes = parseIntList(value);
        else if (key == "out-dir")
            cfg.out_dir = value;
        else if (key == "tag")
            cfg.tag = value;
...
std::vector<int> makeDefaultProbes(int N)
{
    return {N / 2 - 2, N / 2 - 1, N / 2, N / 2 + 1, N / 2 + 2};
}

std::vector<int> makeDefaultElls(int N, int ell_count)
{
    if (ell_count <= 0)
    {
        throw std::runtime_error("ell-count must be positive");
    }

    std::vector<int> ells;
    int ell_max = N / 2;

    for (int k = 1; k <= ell_count; ++k)
    {
        int ell = k * ell_max / ell_count;
        if (ell <= 0)
        {
            ell = 1;
        }
...
std::string makeRunLabel(const ProductionConfig &cfg)
{
    int dt_milli = static_cast<int>(std::round(1000.0 * cfg.dt));

    std::string label =
        "N" + std::to_string(cfg.N) +
        "_chi" + std::to_string(cfg.chi_max) +
        "_steps" + std::to_string(cfg.steps) +
        "_dtm" + std::to_string(dt_milli) +
        "_order" + std::to_string(cfg.order) +
        "_mode" + cfg.mode +
        "_hmeas" + std::to_string(cfg.hm_measure_every) +
        "_Smeas" + std::to_string(cfg.entropy_measure_every);
...
void writeProductionMetadata(const ProductionConfig &cfg,
                             ...
                             const std::vector<int> &ells)
{
    ...
    meta << "chi_max " << cfg.chi_max << "\n";
    meta << "cutoff " << cfg.cutoff << "\n";
    meta << "order " << cfg.order << "\n";
    meta << "theta " << cfg.theta << "\n";
    meta << "alpha " << cfg.alpha << "\n";
    meta << "beta " << cfg.beta << "\n";
    meta << "gamma " << cfg.gamma << "\n";
    meta << "mode " << cfg.mode << "\n";
    meta << "hm_measure_every " << cfg.hm_measure_every << "\n";
    meta << "entropy_measure_every " << cfg.entropy_measure_every << "\n";
    meta << "probes";
    for (int m : probes)
    {
        meta << " " << m;
...
void requireWritableOutput(const std::string &path, bool overwrite)
{
...
void runProductionTEBD(const ProductionConfig &cfg)
{
    bool write_hm = (cfg.mode == "hm" || cfg.mode == "both");
    bool write_entropy = (cfg.mode == "entropy" || cfg.mode == "both");

    if (!write_hm && !write_entropy)
    {
        throw std::runtime_error("mode must be entropy, hm, or both");
    }
    if (cfg.N <= 1)
    {
    ...
    if (cfg.chi_max <= 0)
    {
        throw std::runtime_error("chi must be positive");
    }
    if (cfg.hm_measure_every <= 0 || cfg.entropy_measure_every <= 0)
    {
        throw std::runtime_error("measurement cadences must be positive");
    }

    std::vector<int> probes = cfg.probes.empty() ? makeDefaultProbes(cfg.N) : cfg.probes;
    std::vector<int> ells = cfg.ells.empty() ? makeDefaultElls(cfg.N, cfg.ell_count) : cfg.ells;

    for (int m : probes)
    {
        if (m < 0 || m >= cfg.N)
        {
            throw std::runtime_error("probe m out of range");
        }
    }
    for (int ell : ells)
    {
        if (ell <= 0 || ell >= cfg.N)
        {
            throw std::runtime_error("ell out of range");
        }
    }
    std::sort(ells.begin(), ells.end());

    std::string run_label = makeRunLabel(cfg);
    std::string hm_path = cfg.out_dir + "/ffd_hm_" + run_label + ".csv";
    std::string entropy_path = cfg.out_dir + "/ffd_entropy_" + run_label + ".csv";
    std::string meta_path = cfg.out_dir + "/ffd_meta_" + run_label + ".txt";

    requireWritableOutput(meta_path, cfg.overwrite);
    if (write_hm)
    {
        requireWritableOutput(hm_path, cfg.overwrite);
    }
    if (write_entropy)
    {
        requireWritableOutput(entropy_path, cfg.overwrite);
    }

    writeProductionMetadata(cfg, meta_path, probes, ells);
    ...
    if (write_hm)
    {
        ...
        if (!hm_out)
        {
            throw std::runtime_error("Could not open h_m output: " + hm_path);
        }
        hm_out << "step,t,m,h_real,h_imag,bh_real,bh_imag,"
               << "max_discarded_fraction,total_discarded_weight,max_bond_dim\n";
    }

    if (write_entropy)
    {
        entropy_out.open(entropy_path);
        if (!entropy_out)
        {
            throw std::runtime_error("Could not open entropy output: " + entropy_path);
        }
        entropy_out << "step,t,ell,S,max_discarded_fraction,total_discarded_weight,max_bond_dim\n";
    }

    auto b = makeStaggeredSqrtCouplings(cfg.N, cfg.alpha, cfg.beta, cfg.gamma);
    auto psi = makeTiltedProductState(cfg.N, cfg.theta);
    FFDStepReport last_report;

    auto wall_start = std::chrono::steady_clock::now();

    std::cout << "FFD production TEBD\n";
    std::cout << "run_label " << run_label << "\n";
    std::cout << "N " << cfg.N
              << " dt " << cfg.dt
              << " steps " << cfg.steps
              << " chi " << cfg.chi_max
    ...
    for (int step = 0; step <= cfg.steps; ++step)
    {
        ...
        if (cfg.progress_every > 0 &&
            (step % cfg.progress_every == 0 || step == cfg.steps))
        {
            ...
            std::cout << "step " << step << " / " << cfg.steps
                      << " t " << t
                      << " max_chi " << last_report.max_bond_dim
                      << " max_discarded_fraction " << last_report.max_discarded_fraction
                      << " elapsed_s " << elapsed << "\n";
        }

        if (write_hm &&
            (step % cfg.hm_measure_every == 0 || step == cfg.steps))
        {
            for (int m : probes)
            {
                auto hm = expectationFFDTerm(psi, m);
        ...
        if (write_entropy &&
            (step % cfg.entropy_measure_every == 0 || step == cfg.steps))
        {
            auto entropies = bipartiteEntropiesFirstEllSites(psi, ells);
        ...
        if (step < cfg.steps)
        {
            if (cfg.order < 2)
            {
                last_report = ComplexApplyFFDStep(psi, b, cfg.dt, cfg.chi_max, cfg.cutoff);
            }
            else
            {
                last_report = ComplexApplyFFDStepSecondOrder(psi, b, cfg.dt, cfg.chi_max, cfg.cutoff);
            }
        }
    }
    ...
    if (write_hm)
    {
        std::cout << "saved h_m data to " << hm_path << "\n";
    }
    if (write_entropy)
    {
        std::cout << "saved entropy data to " << entropy_path << "\n";
    }
    std::cout << "saved metadata to " << meta_path << "\n";
}
...
int main(int argc, char **argv)
{
    try
    {
        auto cfg = parseProductionConfig(argc, argv);
        runProductionTEBD(cfg);
    }
    catch (const std::exception &err)
    {
        std::cerr << "error: " << err.what() << "\n";
        std::cerr << "Use --help for command-line options.\n";
        return 1;
    }
